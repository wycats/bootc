...
Source: generated snapshots (see `./scripts/build-system-profile`)
...
To sign commits inside a toolbox, you typically need:

- A terminal pinentry (`pinentry-tty` on Fedora)
- `gpg-agent` configured to use that pinentry
- `GPG_TTY` exported in your shell init

This repo includes a helper script to set that up inside the toolbox:

- `./scripts/toolbox-gpg-setup`

It installs required packages (via `sudo dnf` when available, including `jq`), configures `~/.gnupg/gpg-agent.conf`, and ensures `~/.bashrc` exports `GPG_TTY`.
