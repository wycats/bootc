...
Distrobox exports work by creating wrapper scripts in `~/.local/bin/distrobox` that execute the container binary:

```bash
# ~/.local/bin/distrobox/cargo (generated by distrobox-export)
#!/bin/sh
...
~/.local/bin/distrobox/    # Distrobox exports land here
```
