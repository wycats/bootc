...
3. **Drift Visibility**: `bkt status` shows exactly what's out of sync, reminding you to capture
...
| 3   | [Drift Visibility in Status](#3-drift-visibility)       | Workflow  | 🔴 High   | Not Started |
...
| 6   | [RFC Audit & Cleanup](#6-rfc-audit)                     | Housekeep | 🟢 Low    | Not Started |
...
- [ ] Implement section regeneration from manifest
- [ ] Hook into `bkt dnf install/remove` commands
- [ ] Support COPR repository section
- [ ] Add `bkt containerfile sync` command for manual sync
...
Users install things via GNOME Software, Extension Manager, etc. and forget to run `bkt capture`. There's no reminder.

### Solution

Enhance `bkt status` to show pending capture counts:

```
📦 System Status
  OS: Fedora 41 (Bazzite)
...
- [ ] Add drift counts to `bkt status` output
- [ ] Show specific categories with counts
- [ ] Add actionable suggestion ("Run `bkt capture`")
- [ ] Consider: `bkt status --quiet` for scripting (exit code based on drift)

### Acceptance Criteria

- `bkt status` shows pending capture items
...
RFC statuses don't match implementation reality. RFC-0010 was marked "Draft" but fully implemented.

### Solution

Audit all RFCs and update statuses:

| RFC      | Current Status | Should Be   |
|----------|---------------|-------------|
| RFC-0010 | Draft         | Implemented |
| RFC-0004 | Future        | Review      |

### Deliverables

- [ ] Review each RFC against implementation
- [ ] Update status fields
- [ ] Archive or remove obsolete sections
- [ ] Add "Implementation Notes" sections where design diverged
...
├── 3. Drift Visibility in Status 🔴 QUICK WIN
└── 6. RFC Audit (do alongside other work)
...
4. **Auto-PR**: Changes automatically open a PR to your distribution
5. **Merge**: Review and merge on GitHub
