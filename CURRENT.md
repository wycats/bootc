...
These block users and must be fixed before merge.

| Issue | File | Problem | Fix |
|-------|------|---------|-----|
| ❌ `bootc-apply` broken | `scripts/bootc-apply` | Calls `bkt dnf sync` (doesn't exist) | Change to `bkt system sync` |
| ❌ `bootc-bootstrap` broken | `scripts/bootc-bootstrap` | Calls `shim sync` (doesn't exist) | Change to `bkt shim sync` |
| ❌ Export location wrong | `docs/ARCHITECTURE.md` | Claims `/usr/bin`, reality is `~/.local/bin/distrobox` | Update docs |
